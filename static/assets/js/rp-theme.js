...
(function () {
    ...
    /* `action` is optional: {label, onClick}. A destructive change should be
       reversible from the confirmation itself rather than needing a dialog
       before every removal. */
    window.rpToast = function (msg, icon, action, duration) {
        var t = $('#rp-toast');
        if (!t) return;
        t.innerHTML = '<i class="' + (icon || 'fa-solid fa-circle-check') + '" aria-hidden="true"></i><span></span>';
        ...
        clearTimeout(toastTimer);
        toastTimer = setTimeout(function () { t.classList.remove('show'); }, duration || 3200);
    };
    ...
    /* ---------- category pill strip (products page) ----------
       Filtering itself is server-side now — each pill is a real link to
       /products/?category=… — so all that is left here is keeping the strip
       navigable when it overflows. */
    var catBar = $('#cat-bar');
    ...
    if (catBar && catStrip) {
        ...
        // Bring the current category into view on load, so a deep link does not
        // land with its own pill scrolled off the strip.
        var active = $('.cat-tab.active', catStrip);
        if (active && catStrip.scrollTo) {
            catStrip.scrollTo({
